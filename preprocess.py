...
def build_save_text_dataset_in_shards(src_corpus, tgt_corpus, fields,
                                      corpus_type, opt, datasetConstructor=onmt.io.TextDataset,
                                      shardedCorpusIterator=onmt.io.ShardedTextCorpusIterator):
    '''
    ...
    ret_list = []
    src_iter = shardedCorpusIterator(
                src_corpus, opt.src_seq_length_trunc,
                "src", opt.max_shard_size)
    tgt_iter = shardedCorpusIterator(
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if opt.data_type == 'hierarchicalText':
        return build_save_text_dataset_in_shards(
            src_corpus, tgt_corpus, fields,
            corpus_type, opt, datasetConstructor=onmt.io.HierarchicalTextDataset,
            shardedCorpusIterator=onmt.io.ShardedHierarchicalTextCorpusIterator)
