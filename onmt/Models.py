...
class HierarchicalEncoder(EncoderBase):
    """
    Core hierarchical encoder module.

    Args:
      encoder_inf (:obj:`EncoderBase`): the inferior encoder object
      encoder_sup (:obj:`EncoderBase`): the superior encoder object
      multi<gpu (bool): setup for multigpu support
    """
    def __init__(self, encoder_inf, encoder_sup):
    ...
    def _check_args(self, input, lengths=None, hidden=None):
        ...
        if lengths is not None:
            s_len_plus_1,n_batch_ = lengths.size()
            aeq(n_batch, n_batch_)
    ...
    def forward(self, src, lengths=None):
        ...
        Args:
            src (:obj:`LongTensor`):
               padded sequences of sparse indices `[src_len_sup x src_len_inf x batch x nfeat]`
            lengths (:obj:`LongTensor`): length of each sequence `[src_len_sup+1 x batch]`
               lengths[0] is the length of the superior sequence for each batch
            encoder_state (rnn-class specific):
               initial encoder_state state.
