...
def get_fields(data_type, n_src_features, n_tgt_features):
    ...
    elif data_type == 'hierarchicalText':
        return HierarchicalTextDataset.get_fields(n_src_features, n_tgt_features)
    elif data_type == 'img':
...
def get_num_features(data_type, corpus_file, side):
    ...
    if data_type == 'text':
        ...
        return HierarchicalTextDataset.get_num_features(corpus_file, side)
    elif data_type == 'img':
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    for path in train_dataset_files:
        ...
        for ex in dataset.examples:
            for k in fields:
                if not isinstance(fields[k],torchtext.data.field.NestedField):
                    val = getattr(ex, k, None)
                    if val is not None and not fields[k].sequential:
                        val = [val]
                    counter[k].update(val)
                else: #need to process each el in the sequence
                    val = getattr(ex, k, None)
                    if val is not None:
                        for el in val:
                            if el is not None and not fields[k].nesting_field.sequential:
                                el = [el]
                            counter[k].update(el)
    ...
    if data_type == 'text' or data_type == 'hierarchicalText':
